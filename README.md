...
最新版本:1.14  更新日期2024 / 4 / 25 (更新非常快,请关注获取最新特性)
    ...
    v1.10
    加入 快捷键功能.  "默认呼出主界面快捷键":ctrl+alt+X    以及"默认单锁模式下解冻"快捷键: ctrl+alt+E (已删除此快捷键,原件可见v1.13)  掌机用户可映射至某个自定义键上. 键盘鼠党就随意啦.
...
评论区里也说得很到位,就是这么回事.
     ...
     360, 还有微软那些什么基于声誉保护,实时保护是很令人无语, 有一次下个软件死活打不开, 也没个选项, 强制性拦截.,把人整疯,后来在设置里钻来钻去才找到开关...前些天调试雪藏的时候, 不打包就没事,一打包就报毒..我原本以为是导入某些库导致报毒, 看了上面视频才发现和写的内容毫无关系, 仅仅用pyinstaller打包exe就能报毒, pyinstaller也算py用户最常用打包工具了(没有之一), 所以最好的方式就是关掉或者白名单.   不然真心无解.  我为啥换成火绒?..之前我用360,并不是用他的杀毒功能, 用的是各种他附带的小工具,断网急救箱啊,清理磁盘啊等等(我这人比较懒,我知道有其他针对性的软件都能做到这些,但懒得找,直接在360工具包里用), 后来发现各种常用的软件被他强行关掉开机自启, 这就算了,像pixpin这种软件甚至被删掉各种dll组件...真的是添乱第一名,  火绒就很巴适了,至少不那么霸道.  话说回来,反正我常年一直都用pyinstaller打包, 自用没问题(可能我关闭微软自带那些东西很久了,如果没有某个机友的反馈我还真不知道是pyinstaller的问题.),听讲也有其他好几种打包方式, 部署环境较为麻烦, 但如果如果有更好的py打包方式可以推荐.如果不打包咋分享?哪怕以后我开源了,扔源代码给用户,没环境用户也没法直接跑啊,所有打包是绕不开的.
