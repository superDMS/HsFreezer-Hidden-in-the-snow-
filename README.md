...
(每天有n多反馈,雪藏会不间断优化,适应更多需求面,样本足够多才能足够的完善,没反馈还真不知道有那么多活久见的问题,哈哈,咱的框架拓展性是充分预留的.)
...
最新版本:---1.39 1.37重大内核级更新,冻结能力大幅提升,可冻结包含但不仅限于:LOL,崩坏3,无畏契约等等任务管理器都搞不定的网游进程以及之前你冻不住的进程,之前说冻到后台还有声音的朋友赶紧来尝试(本人拥有大量群友提供远程调试环境,基本都是马上现场解决,太多游戏就不一一提名了,日均解决数十款不同游戏的细节问题.).1.优化:进阶/休闲面板切换流畅的.2.改进:进阶/休闲面板进程列表统一.3.优化:雪藏在前台时CPU占用逻辑.4.优化:列表响应反馈.---更新日期2024 / 5 / 12 (最近一个月更新非常快,因为大批量密集测试,疯狂改bug中,如果有异常,也许隔一会来着看看更新日志就修复了,请关注获取最新特性)---
...
---V1.37---(重大更新,这个版本一定得更!)
1.内核大更新, 找到众多关键反调试线程,冻结能力大幅度提升,包含但不仅限于:LOL,永劫无间,无畏契约,原神,崩铁,cs:go,崩坏3,我的世界等等等等,保证不触发反作弊等,因为无修改数据操作,这个有大量玩家已实测,可放心食用.(有小蓝熊的游戏除外,小蓝熊要改标志位,我测试已成功,但这个操作过于敏感,暂不放出.)
