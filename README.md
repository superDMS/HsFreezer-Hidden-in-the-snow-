...
关于敏感操作报毒 (很玄学,有时候报有时候不报,但还是得剖析说明情况):
    两方面原因:1.系统级跨进程高权限操作,被一刀切判定 2.python的pyinstaller打包机制问题.
    
    高权限操作就不用说了,类似风灵月影修改器的情况,频繁跨不同游戏进程进行操作肯定会被视为敏感操作的,
    ...
    至于pyinstaller打包,本人刚开始也纳闷以为是导入各种包导致,后来搜了才发现原来这是正常现象
