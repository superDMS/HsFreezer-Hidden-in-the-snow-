...
关于敏感操作报毒 (很玄学,有时候报有时候不报,但还是得剖析说明情况):
    有用户反馈1.37版本以后微软自带报毒.
    两方面原因:1.系统级跨进程高权限操作 2.python的pyinstaller打包机制问题.
    
    高权限操作就不用说了,类似风灵月影修改器的情况,频繁跨不同游戏进程进行操作肯定会被视为敏感操作的,
    这一般是输入法和杀软执行这些才正常,咱这软件还没一个清晰的权限归类,所以被判定很正常,加上又没弄签名,没氪金是有点小问题
    ...
    我只负责专心打磨"雪藏"这把"武器",至于各种安全软件对"雪藏"怎么判定我就管不着了,咱写个软件都不容易了,还有心思和杀软对抗?
