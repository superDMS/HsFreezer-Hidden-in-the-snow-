...
最新版本:---1.42 1.修复:隐藏雪藏(或最小化到托盘)后点击任务栏图标会导致多开雪藏的问题2.新用户启动默认关闭妙控拖拽.---更新日期2024 / 5 / 12 (最近一个月更新非常快,因为大批量密集测试,疯狂改bug中,如果有异常,也许隔一会来着看看更新日志就修复了,请关注获取最新特性)---
...
急冻或出现异常的游戏(如黑屏等):
先弄清楚目标程序对应的exe名字叫啥拿英雄联盟举例:英雄联盟的主界面是:LeagueClientUx.exe,而不是LeagueClient.exe.然后进入到游戏后,又是另一个进程,叫League of Legends.exe.
有些网游进程比较古怪,这个得自己多加尝试,后台有n个进程迷惑你.单机往往就比较简单,就一个,很好选择.还有就是exe的名称,任务管理器>右键>属性的那个名称,而非标题上的名称,当然,exe名称往往也和标题名称有关联,在全局模式尝试搜索一般都能搜到.
