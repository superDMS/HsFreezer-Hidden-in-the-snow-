...
这是一款让你可以随心冻结游戏(游戏暂停)的软件,想玩就玩,想停就停.

雪藏会让你深刻感受到--内存,原来是这么用的.

作者:大陌DM   Q:269653764 
# HsFreezer(Hidden in the snow)
最新版本:---1.20---  更新日期2024 / 4 / 29 (最近一个月更新非常快,请关注获取最新特性)
...
(原神党久等了, 原神随意冻结,冻结时长太长解冻仅会重连而已,非常快速的还原. 实测联网类游戏可用)
    ...
    v0.01
    弹窗模式初代雪藏于2017年建立 (当时80行代码,自用, 迭代至今, 现GUI界面+逻辑处理代码总量13700+行,持续增加中.....)
    ...
    性能模式: 固定当下列表, 不再进行扫描检测,  在找到目标进程后, 可开启性能模式达到极致性能(当然, 实时模式也没啥消耗,但处女座嘛....)
    2.优化急冻/解冻性能, 消除迟滞感, 
    3.优化列表刷新机制, 不再频闪.
